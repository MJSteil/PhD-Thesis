...
with open('he_P1.json') as json_file:
    dataP1 = json.load(json_file)
    
with open('he_P2.json') as json_file:
...
t0=min(min(dataP1['ti']),min(dataP2['ti']))
t1=max(max(dataP1['ti']),max(dataP2['ti']))
dt=t1-t0
c=get_color_list(colors[0],colors[1],101)
cP1=[c[math.ceil((t-t0)/dt*100)] for t in dataP1['ti']]
cP2=[c[math.ceil((t-t0)/dt*100)] for t in dataP2['ti']]
